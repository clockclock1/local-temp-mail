...
export const sendMailToVerifyAddress = async (
    ...
    try {
        ...
        console.warn("Falling back to structured SEND_MAIL payload", error);
    }
...
const sendMailBySmtp = async (
    ...
    try {
        ...
        console.warn("Falling back to nodemailer SMTP transport", error);
    }

    const smtpTransport = nodemailer.createTransport({
        host: (smtpOptions as any).host,
        port: Number((smtpOptions as any).port || 25),
        secure: Boolean((smtpOptions as any).secure),
        auth: (smtpOptions as any).auth,
    });
